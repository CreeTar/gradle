...
public class DefaultExecutionPlan implements ExecutionPlan {
    ...
    private void onOrderingCycle(Node successor, Node node) {
        CachingDirectedGraphWalker<Node, Void> graphWalker = new CachingDirectedGraphWalker<>((node1, values, connectedNodes) -> {
            connectedNodes.addAll(node1.getDependencySuccessors());
            if (node1 instanceof TaskNode) {
                TaskNode taskNode = (TaskNode) node1;
        ...
        DirectedGraphRenderer<Node> graphRenderer = new DirectedGraphRenderer<>(
            (node12, output) -> output.withStyle(StyledTextOutput.Style.Identifier).text(node12),
            (node13, values, connectedNodes) -> {
                for (Node dependency : firstCycle) {
                    if (node13.hasHardSuccessor(dependency)) {
                ...
                TaskPropertyUtils.visitProperties(propertyWalker, task, new PropertyVisitor.Adapter() {
                    ...
                    public void visitOutputFileProperty(final String propertyName, boolean optional, final PropertyValue value, final OutputFilePropertyType filePropertyType) {
                        withDeadlockHandling(
                            ...
                            () ->
                                FileParameterUtils.resolveOutputFilePropertySpecs(task.toString(), propertyName, value, filePropertyType, fileCollectionFactory, outputFilePropertySpec ->
